...
def TCGA_CGGA_integrate():
    ...
    cgga_gene_cols = set(cgga.columns) - set(metadata_cols)
    tcga_gene_cols = set(tcga.columns) - set(metadata_cols)

    # Find common genes in both datasets
    common_genes = cgga_gene_cols.intersection(tcga_gene_cols)

    # Keep only common genes and metadata columns
    cgga_filtered = cgga[list(metadata_cols) + list(common_genes)]
    tcga_filtered = tcga[list(metadata_cols) + list(common_genes)]

    # Combine the datasets
    combined_data = pd.concat([cgga_filtered, tcga_filtered], ignore_index=True)
...
def main():
    ...
    outdir = "/home/mkh062/Desktop/scratch/TCGA_project/processed_data/TCGA_CGGA_integrated/cox_results"
    os.makedirs(outdir, exist_ok=True)
    ...
    for threshold in thresholds_fixed:
        for ds_data, ds_name in [(tcga_cgga, "CGGA_TCGA")]:
    ...
    for (low,high) in thresholds_range:
        for ds_data, ds_name in [(tcga_cgga, "CGGA_TCGA")]:
            res = limma(ds_data.copy(),
                                       ...
                                       outdir=output_dir)
    analyze_dataset(tcga_cgga, "CGGA_TCGA", outdir, alpha_fdr=0.1)
